...
class RecentViewProductCard extends StatelessWidget {
  final RecentViewProdutItem product;
  RecentViewProductCard({Key? key, required this.product}) : super(key: key);
  final landingPageController = Get.put(LandingPageController());
  final recentController = Get.put(RecentViewController());
                    ...
                    Row(
                      children: [
                        IconButton(
                            onPressed: () async {
                              final restaurantId =
                                  await landingPageController.getRestaurantId();
                              if (restaurantId == product.storeId ||
                                  restaurantId != 0) {
                                if (landingPageController.contactId.value ==
                                  ...
                                  if (result) {
                                    await landingPageController
                                        .setRestaurantId(product.storeId!);
                                    Get.snackbar(
                                      "Cart",
